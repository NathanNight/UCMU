...
function runAuthIntro(){
  ...
  setTimeout(()=>$('#authForm')?.classList.add('built'),1780);
...
function renderAuth(){
  ...
  $$('.auth-tab').forEach(b=>b.onclick=()=>{state.mode=b.dataset.mode;renderAuth();});
  $('.pass-eye').onclick=()=>{const p=$('#password');p.type=p.type==='password'?'text':'password'};
