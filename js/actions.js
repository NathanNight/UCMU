...
function bindDrag(){let dragged=null,dragId=null,lastOver=null,dropFolderId=null;
  const selector='[data-chat-id]';
  function chatItems(scope=document){return Array.from(scope.querySelectorAll(selector)).filter(el=>!el.classList.contains('dragging'))}
  function measure(list){const m=new Map();list.forEach(el=>m.set(el,el.getBoundingClientRect()));return m}
  function flip(list,before){list.forEach(el=>{const a=el.getBoundingClientRect(),b=before.get(el);if(!b)return;const dx=b.left-a.left,dy=b.top-a.top;if(!dx&&!dy)return;el.style.transition='none';el.style.transform=`translate(${dx}px,${dy}px)`;requestAnimationFrame(()=>{el.style.transition='transform .24s cubic-bezier(.16,.86,.22,1)';el.style.transform=''})})}
  function movePreview(over,e){if(!dragged||!over||over===dragged||lastOver===over)return;const list=over.parentElement;if(!list)return;const before=measure(chatItems(list));const r=over.getBoundingClientRect();const after=(e?.clientY||0)>r.top+r.height/2;list.insertBefore(dragged,after?over.nextSibling:over);flip(chatItems(list),before);lastOver=over}
  ...
  document.addEventListener('dragstart',e=>{const item=e.target.closest(selector);if(!item)return;dragged=item;dragId=item.dataset.chatId;state.drag=dragId;document.body.classList.add('drag-live');item.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',dragId);setTimeout(()=>item.classList.add('chat-source-hidden'),0)});
  document.addEventListener('dragend',()=>{dragged?.classList.remove('dragging','chat-source-hidden');dragged=null;dragId=null;lastOver=null;state.drag=null;document.body.classList.remove('drag-live');$$('.folder-drop-target').forEach(x=>x.classList.remove('folder-drop-target'))});
  document.addEventListener('dragover',e=>{if(!state.drag)return;const overChat=e.target.closest(selector);const overFolder=e.target.closest('[data-folder-id]');if(!overChat&&!overFolder)return;e.preventDefault();if(overFolder&&!overChat)markFolder(overFolder);else markFolder(null);if(overChat)movePreview(overChat,e)});
  document.addEventListener('drop',e=>{if(!state.drag)return;e.preventDefault();const folder=e.target.closest('[data-folder-id]');const chat=e.target.closest(selector);const from=state.drag;if(folder&&!chat){addChatToFolder(from,folder.dataset.folderId)}else if(chat){reorderChatsFromDom()}renderChats();requestAnimationFrame(()=>{const n=$(`[data-chat-id="${from}"]`);if(n)appear(n)})})}
function reorderChatsFromDom(){const visible=Array.from(document.querySelectorAll('#chatList>[data-chat-id]')).map(el=>el.dataset.chatId);if(!visible.length)return;const map=new Map(state.chats.map(c=>[c.id,c]));const placed=new Set(visible);const reordered=visible.map(id=>map.get(id)).filter(Boolean);state.chats.forEach(c=>{if(!placed.has(c.id))reordered.push(c)});state.chats=reordered}
