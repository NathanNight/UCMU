...
function bindDrag(){let dragged=null,dragId=null,lastOver=null,lastAfter=null,dropFolderId=null;
  const selector='[data-chat-id]';
  function cleanup(){dragged?.classList.remove('dragging','chat-source-hidden');dragged=null;dragId=null;lastOver=null;lastAfter=null;state.drag=null;document.body.classList.remove('drag-live');$$('.folder-drop-target').forEach(x=>x.classList.remove('folder-drop-target'))}
  ...
  function movePreview(over,e){if(!dragged||!over||over===dragged)return;const list=over.parentElement;if(!list)return;const r=over.getBoundingClientRect();const after=(e?.clientY||0)>r.top+r.height/2;if(lastOver===over&&lastAfter===after)return;const before=measure(chatItems(list));list.insertBefore(dragged,after?over.nextSibling:over);flip(chatItems(list),before);lastOver=over;lastAfter=after}
  ...
  document.addEventListener('drop',e=>{if(!state.drag)return;e.preventDefault();const folder=e.target.closest('[data-folder-id]');const chat=e.target.closest(selector);const from=state.drag;if(folder&&!chat){addChatToFolder(from,folder.dataset.folderId)}else if(chat){reorderChatsFromDom()}cleanup();renderChats()})}
