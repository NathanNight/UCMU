...
function injectStyles(){
  ...
  style.id='ucmu-member-profile-style';
  style.textContent=`
    ...
    #profileModal .ucmuMemberActions button{width:100%!important;min-height:38px!important}
    #profileModal .dangerMemberKick{background:linear-gradient(135deg,#d71920,#6f0b10)!important;color:#fff!important}
  `;
...
function openMemberProfile(uid){
  ...
  box.innerHTML=`
    ...
    <div class="profileStub ucmuProfileCard">
      <div class="ava"></div>
      <div class="ucmuProfileInfo">
        <b id="profileName">${esc(displayName(p,uid))}</b>
        <p>${esc(displaySub(p)||'Профиль пока пустой')}</p>
      </div>
...
async function writeToMember(uid){
  const p=profileFor(uid);
  hide($('#profileModal'));
  await createOrOpenChatWithUsers([{uid,...p}]);
}
...
async function kickMember(uid){
  ...
  try{
    await updateDoc(doc(await db(),'chats',c.id),{
    ...
    toast('Не удалось удалить участника:\n'+(err.message||err.code||err));
  }
}
...
export function initMemberProfile(){
  ...
  document.addEventListener('click',async e=>{
    ...
    if(chat)hide($('#profileModal'));

    if(e.target.closest?.('#closeProfile')){hide($('#profileModal'));return}

    const uid=e.target.closest?.('[data-real-member-id]')?.dataset.realMemberId;
    if(uid){e.preventDefault();openMemberProfile(uid);return}

    const more=e.target.closest?.('[data-member-more]')?.dataset.memberMore;
    if(more){toast('Полный профиль будет отдельным экраном.');return}

    const write=e.target.closest?.('[data-member-write]')?.dataset.memberWrite;
    if(write){await writeToMember(write);return}

    const kick=e.target.closest?.('[data-member-kick]')?.dataset.memberKick;
    if(kick){await kickMember(kick);return}
  },true);
}
