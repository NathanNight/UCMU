...
function localCascadeClear(){
  ...
  nodes.forEach((n,i)=>setTimeout(()=>{n.style.transition='transform .32s cubic-bezier(.16,.86,.22,1),opacity .25s ease,filter .25s ease,max-height .32s ease,margin .32s ease,padding .32s ease';n.style.transform='translateX(-22px) scale(.96)';n.style.opacity='0';n.style.filter='blur(8px)';n.style.maxHeight='0px';n.style.marginTop='0';n.style.marginBottom='0';n.style.paddingTop='0';n.style.paddingBottom='0';n.style.overflow='hidden'},i*32));
  setTimeout(()=>{if(state.messages[chatId])state.messages[chatId]=state.messages[chatId].filter(m=>!ids.includes(m.id));if(window.__ucmuClearAnimatingChatId===chatId)delete window.__ucmuClearAnimatingChatId;renderFeed()},Math.min(1000,ids.length*32+420));
}
