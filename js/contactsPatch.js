...
import {renderAll,renderFeed,markMessageLocallyGone} from './render.js';
...
function openCenterModal({kicker='CONFIRM',title='Подтверждение',text='',html='',confirm='ПОДТВЕРДИТЬ',danger=false,onConfirm}){
  ...
  requestAnimationFrame(async()=>{shade.classList.add('show');modal.classList.add('show');await wait(80);await typeInto($('#modalKicker'),kicker,11);await wait(42);await typeInto($('#modalTitle'),title,13);await wait(42);await typeInto($('#modalText'),text,8);await wait(50);dyn.classList.add('show');await wait(115);modal.classList.add('seq-ready')});
}
function closeCenterModal(){const shade=$('#modalShade'),modal=$('#centerModal');shade.classList.remove('show');modal.classList.remove('show','seq-ready');setTimeout(()=>{hide(shade);hide(modal);$('#modalDynamic').innerHTML=''},150)}
...
function localCascadeClear(){
  ...
  nodes.forEach((n,i)=>setTimeout(()=>{n.style.transition='transform .28s cubic-bezier(.16,.86,.22,1),opacity .22s ease,filter .22s ease,max-height .28s ease,margin .28s ease,padding .28s ease';n.style.transform='translateX(-22px) scale(.96)';n.style.opacity='0';n.style.filter='blur(8px)';n.style.maxHeight='0px';n.style.marginTop='0';n.style.marginBottom='0';n.style.paddingTop='0';n.style.paddingBottom='0';n.style.overflow='hidden'},i*28));
  setTimeout(()=>{if(state.messages[chatId])state.messages[chatId]=state.messages[chatId].filter(m=>!ids.includes(m.id));renderFeed()},Math.min(900,ids.length*28+360));
}
